...
from (select inicio_real.patient_id,
             ...
             DATE_FORMAT(inicio_real.data_levant, '%d/%m/%Y') as 'data_levant'

      FROM (Select fp_levantamentos.*
            /************************ MDC: FARMAC / Farmácia Privada  165177  **************************/
            from (Select dd.*, levantamentos.data_levant
                  from (SELECT e.patient_id,
                        ...
                        GROUP BY patient_id) dd
                           left join (SELECT patient_id, encounter_datetime as data_levant
                                      ...
                                      WHERE row_number <=12) levantamentos
                                     on levantamentos.patient_id = dd.patient_id) fp_levantamentos
            order by patient_id, data_levant desc) inicio_real
