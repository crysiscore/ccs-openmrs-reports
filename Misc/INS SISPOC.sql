...
(SELECT
	...
	FROM (select patient_id, data_inicio,  data_usar data_fim
		...
		LEFT JOIN
			(
				...
				FROM 	encounter e
                INNER JOIN
                         ( SELECT e.patient_id,min(encounter_datetime) encounter_datetime
                         FROM encounter e
                         INNER JOIN obs o ON e.encounter_id=o.encounter_id
    ...
    /************************** keypop concept_id = 23703 ****************************/
               LEFT JOIN
		(SELECT ultimavisita.patient_id,ultimavisita.encounter_datetime data_keypop,
        CASE o.value_coded
					WHEN '23903'  THEN 'Diagnostico Negativo'
					WHEN '1366' THEN 'Obitou'
					WHEN '1705' THEN 'Recaida'
					WHEN '1706'  THEN 'Transferido Para'
					WHEN '1707' THEN 'Abandono'
                   	WHEN '1709' THEN 'Suspender Tratamento'

				    ELSE '' END AS permanencia
			FROM
			...
			INNER JOIN encounter e ON e.patient_id=ultimavisita.patient_id
			INNER JOIN obs o ON o.encounter_id=e.encounter_id
			WHERE o.concept_id=23703 AND o.voided=0 AND e.encounter_datetime=ultimavisita.encounter_datetime AND
			e.encounter_type IN (6,9,34,35)  AND e.location_id=:location
			GROUP BY e.patient_id
		) permanencia_tarv ON permanencia_tarv.patient_id=inicio_real.patient_id
...
SELECT 	e.patient_id,
        ...
        LEFT JOIN(
		    ...
		    (
            SELECT 	cd4_min.patient_id, MIN(cd4_min.encounter_datetime) AS encounter_datetime
        ...
        LEFT JOIN(
            ...
            FROM encounter e INNER JOIN
		    (
            SELECT 	cd4_min.patient_id, MAX(cd4_min.encounter_datetime) AS encounter_datetime
            FROM ( SELECT e.patient_id, o.value_numeric , encounter_datetime
					FROM encounter e
							INNER JOIN obs o ON o.encounter_id=e.encounter_id
					WHERE 	e.voided=0  AND e.location_id=:location  AND
		...
		) cd4_final ON cd4_final.patient_id =  inicio_real.patient_id
		/****************** ****************************  CD4  Percentual  Inicial*****************************************************/
        LEFT JOIN(
            SELECT e.patient_id, o.value_numeric,e.encounter_datetime
            FROM encounter e INNER JOIN
		    (
            ...
            FROM (
					...
					FROM encounter e
							INNER JOIN obs o ON o.encounter_id=e.encounter_id
					WHERE 	e.voided=0  AND  e.location_id=:location  AND
							o.voided=0 AND o.concept_id=730 AND e.encounter_type in (6,9,13,53)  )cd4_min
			GROUP BY patient_id ) cd4_temp
            ON e.patient_id = cd4_temp.patient_id
            INNER JOIN obs o ON o.encounter_id=e.encounter_id
            WHERE e.encounter_datetime=cd4_temp.encounter_datetime AND
		...
		) cd4_perc_final ON cd4_perc_final.patient_id =  inicio_real.patient_id
        ...
        SELECT 	e.patient_id,
              ...
              o.comments as valor_comment,
                ...
                FROM  encounter e
                INNER JOIN	(
							SELECT 	e.patient_id,MIN(encounter_datetime) AS data_cv
							FROM encounter e INNER JOIN obs o ON e.encounter_id=o.encounter_id
                ...
                GROUP BY e.patient_id
		) cv_inicial ON cv_inicial.patient_id =  inicio_real.patient_id
        ...
        SELECT 	e.patient_id,
              ...
              o.comments as valor_comment,
				...
				o.voided=0 AND 	o.concept_id IN( 856, 1305) /* AND  e.encounter_datetime <= :endDate */
                GROUP BY e.patient_id
		) cv_final ON cv_final.patient_id =  inicio_real.patient_id
                                        ...
                                        from (select saidas_por_transferencia.patient_id, max(data_estado) data_estado
                                              from (select distinct max_estado.patient_id, max_estado.data_estado
                                                    ...
                                                    where e.encounter_type = 21
                                                      ...
                                                      and e.location_id = :location) saidas_por_transferencia
                                              group by patient_id) saidas_por_transferencia
                    ) transferido ON transferido.patient_id = inicio_real.patient_id
