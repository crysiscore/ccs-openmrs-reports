...
( SELECT 	inicio_real.patient_id,
	...
	FROM ( 
     ...
     /************************* LACTANTES *********************************************/
     INNER JOIN  (	select patient_id,max(date_enrolled) as date_enrolled
		from
...
/****************************************************************************/
	WHERE
          ...
          cv_qualitativa.data_cv_qualitativa is null and  (
          
          cv.patient_id is null 
          or 
          (  ) 
         ...
         /* 3. Não ter resultados de CV nos ultimos 6 meses*/
          ( cv_qualitativa.carga_viral_qualitativa is null and  cv.patient_id is null  and data_ult_pedido_cv is not null and DATEDIFF(:endDate,data_ult_pedido_cv)/30 > 2) 
           ...
           (cv.data_ult_cv is not null and cv.data_ult_cv <  inicio_real.date_enrolled   and DATEDIFF(:endDate,data_ult_pedido_cv)/30 > 2  )  or 
         (cv.data_ult_cv is not null and cv.data_ult_cv <  inicio_real.date_enrolled   and data_ult_pedido_cv is  null  ) 
