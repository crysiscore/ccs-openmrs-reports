...
FROM (SELECT patient_id, data_inicio
    ...
    /**********      Lactantes Diagnosticadas  na CPP    ***************/
         INNER JOIN (
    /**********            LACTANTE            ***************/
   SELECT patient_id, MIN(data_parto) data_parto
...
WHERE timestampdiff(year, birthdate, date(DATE_ADD(now(), INTERVAL (-WEEKDAY(now())) DAY))) >= 15
  and ( datediff(date(DATE_ADD(now(), INTERVAL (-WEEKDAY(now())) DAY)), lactante_real.data_parto) between 21 and 28 or
       datediff(date(DATE_ADD(now(), INTERVAL (-WEEKDAY(now())) DAY)), lactante_real.data_parto ) between 45 and 52 or
       datediff(date(DATE_ADD(now(), INTERVAL (-WEEKDAY(now())) DAY)), lactante_real.data_parto) between 76 and 83 or
       datediff(date(DATE_ADD(now(), INTERVAL (-WEEKDAY(now())) DAY)), lactante_real.data_parto ) between 263 and 270)
