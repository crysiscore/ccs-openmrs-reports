select activos.* ,pn.given_name, pn.middle_name ,pn.family_name, tratamento_tb.data_ult_consulta, tratamento_tb.tratamento_tb, peso.peso
...
left join person p on p.person_id=activos.patient_id
left join person_name  pn on pn.person_id=p.person_id

/**            TRATAMENTO TB             **/
left join (
select e.patient_id, e.encounter_datetime as data_ult_consulta_reg_tb,
case o.value_coded
           when 1256 then 'INICIAR'
            when  1257 then 'CONTINUAR'
            when 1267 then 'FIM'
            ELSE 'OUTRO'
            END  as tratamento_tb
from
(
select  e.patient_id, max(encounter_datetime) data_ult_consulta_registo_tb
from encounter e
INNER join obs o on e.encounter_id=o.encounter_id
where
    e.voided =0 and o.voided=0 and
    e.encounter_type in (6,9) and
    o.concept_id=1268 and
    e.location_id = :location

group by e.patient_id ) ult_consulta_reg_tb

inner join encounter e on e.patient_id=ult_consulta_reg_tb.patient_id and e.encounter_datetime=ult_consulta_reg_tb.data_ult_consulta_registo_tb
inner join obs o on e.encounter_id=o.encounter_id
where o.concept_id=1268 and e.encounter_type in (6,9) and e.voided=0 and o.voided=0  and e.location_id = :location
group by e.patient_id

    ) tratamento_tb on tratamento_tb.patient_id= activos.patient_id
