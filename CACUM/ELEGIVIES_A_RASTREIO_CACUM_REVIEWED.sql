...
/********************************** criterio 1:   ≥12 mêses em TARV ******************************************************/
            where DATEDIFF(:endDate,activos.data_inicio)/30 >=12
        ...
        criterio_1.patient_id not in (
			...
			FROM encounter e
               INNER JOIN (
                   ...
                   FROM 	   encounter e
                                inner join obs o on o.encounter_id =e.encounter_id
								where e.encounter_type IN (6,9) AND e.voided=0 AND o.voided=0 AND o.concept_id=165174 AND o.value_coded in (165314,165179,165265,165264,23729,165321,
			                                                                                     23731,23888,23726,165340,165178,165319,165318,23730,165315,165316,23725,165177,
			                                                                                     165317,23727,23724,165320,23732)
...
(
	...
	-- where  modelodf.data_modelo between :startDate and :endDate -- and modelodf.value_coded <> 1267

) activos  -- where data_modelo between :startDate and :endDate